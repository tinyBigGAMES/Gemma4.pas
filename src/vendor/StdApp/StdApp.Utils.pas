...
type
  ...
  public
    ...
    class function  AppBasedPath(const APath: string;
      const ABasePath: string = ''): string; static;
...
{ AppBasedPath }
class function TUtils.AppBasedPath(const APath: string;
  const ABasePath: string): string;
begin
  if TPath.IsPathRooted(APath) then
    Result := APath
  else if ABasePath <> '' then
    Result := TPath.Combine(ABasePath, APath)
  else
